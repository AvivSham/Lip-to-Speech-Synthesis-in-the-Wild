...
class MultiDataset(Dataset):
    ...
    def build_file_list(self, lrs2, mode):
        file_list, paths = [], []
        crops = {}
        lrs2_mode = "main"

        ## LRS2 crop (lip centered axis) load ##
        file = open(f"./data/LRS2/LRS2_crop/preprocess_{lrs2_mode}.txt", "r")
        content = file.read()
        file.close()
        for i, line in enumerate(content.splitlines()):
            ...
            file = split[0]
            crop_str = split[1][4:]
            crops[f"{lrs2_mode}/{file}"] = crop_str
        ## LRS2 file in crop ##
        file = open(f"./data/LRS2/{mode}.txt", "r")
        content = file.readlines()
        file.close()
        for file in content:
            file = file.strip().split()[0]
            file = f"{lrs2_mode}/{file}"
            if file in crops:
                file_list.append(file)
                paths.append(f"{lrs2}/{file}")
        if mode == "train":
            file = open(f"./data/LRS2/LRS2_crop/preprocess_pretrain.txt", "r")
            content = file.read()
            file.close()
            for i, line in enumerate(content.splitlines()):
                split = line.split(".")
                file = split[0]
                crop_str = split[1][4:]
                crops[f"pretrain/{file}"] = crop_str
            ## LRS2 file in crop ##
            file = open(f"./data/LRS2/pretrain.txt", "r")
            content = file.readlines()
            file.close()
            for file in content:
                file = file.strip().split()[0]
                file = f"pretrain/{file}"
                if file in crops:
                    file_list.append(file)
                    paths.append(f"{lrs2}/{file}")
    ...
    def build_tensor(self, frames, crops):
        ...
        if self.augmentations:
            ...
            temporalVolume[
                ...
                np.maximum(0, x_s) : np.minimum(112, x_s + 56),
            ] = 0.0
    ...
    def __getitem__(self, idx):
        ...
        audio, _ = librosa.load(
            file_path.replace("LRS2-BBC", "LRS2-BBC_audio") + ".wav", sr=16000
        )
        ...
        if num_v_frames > self.max_v_timesteps:
            print(
                f"Cutting Video frames off. Requires {num_v_frames} frames: {file}. But Max_timestep is {self.max_v_timesteps}"
            )
            vid = vid[: self.max_v_timesteps]
            num_v_frames = vid.size(0)
            num_a_frame = round(
                num_v_frames / self.info["video_fps"] * self.info["audio_fps"]
            )
        ...
        if self.sample_window:
            (
                ...
                window_size,
                num_a_frames,
            ) = self.extract_window(vid, melspec, spec, audio, self.info)
        else:
            start_frame = 0
            window_size = vid.size(0)
            num_a_frames = melspec.size(2)
            melspec = nn.ConstantPad2d(
                (0, window_size * 4 - melspec.size(2), 0, 0), 0.0
            )(melspec)
            spec = nn.ConstantPad2d((0, window_size * 4 - spec.size(2), 0, 0), 0.0)(
                spec
            )
        ...
        audio_length = int(
            window_size / self.info["video_fps"] * self.info["audio_fps"]
        )
    ...
    def get_pretrain_words(self, content):
        ...
        num_words = min(random.randint(4, 20), len(words))
    ...
    def encode(self, content):
        ...
        if len(encoded) > self.max_text_len:
            print(
                f"Max output length too short. Required {len(encoded)}. But Max text_length is {self.max_text_len}"
            )
    ...
    def extract_window(self, vid, mel, spec, aud, info):
        ...
        window_size = min(
            random.randint(self.min_window_size, self.max_window_size), vid.size(0)
        )
        ...
        if mel.size(2) < mel_window_size:
            mel = nn.ConstantPad2d((0, mel_window_size - mel.size(2), 0, 0), 0.0)(mel)
            spec = nn.ConstantPad2d((0, mel_window_size - spec.size(2), 0, 0), 0.0)(
                spec
            )
            ...
            [
                ...
                torch.zeros(
                    [
                        1,
                        int(
                            window_size / info["video_fps"] * info["audio_fps"]
                            - aud.size(1)
                        ),
                    ]
                ),
    ...
    def collate_fn(self, batch):
        ...
        for i, (
            ...
            padded_melspec.append(
                nn.ConstantPad2d((0, max_spec_length - melspec.size(2), 0, 0), 0.0)(
                    melspec
                )
            )
            padded_spec.append(
                nn.ConstantPad2d((0, max_spec_length - spec.size(2), 0, 0), 0.0)(spec)
            )
            sp_mels.append(
                nn.ConstantPad2d((0, 100 - sp_mel.size(2), 0, 0), 0.0)(sp_mel)
            )
            padded_audio.append(
                torch.cat([audio, torch.zeros([max_aud_length - audio.size(0)])], 0)
            )
    ...
    def inverse_mel(self, mels, mel_len, stft):
        ...
        for kk, mel in enumerate(mels):
            ...
            wav = (
                griffin_lim(spec_from_mel, stft.stft_fn, 60).squeeze(1).squeeze(0)
            )  # L
    ...
    def inverse_spec(self, specs, mel_len, stft):
        ...
        for kk, spec in enumerate(specs):
            ...
            wav = (
                griffin_lim(spec.squeeze(1), stft.stft_fn, 60).squeeze(1).squeeze(0)
            )  # L
            wav = wav.cpu().numpy() if wav.is_cuda else wav.numpy()
            wav = self.deemphasize(wav)
            wav = np.clip(wav, -1, 1)
    ...
    def plot_spectrogram_to_numpy(self, mels):
        ...
        im = ax.imshow(
            np.squeeze(mels, 0), aspect="auto", origin="lower", interpolation="none"
        )
