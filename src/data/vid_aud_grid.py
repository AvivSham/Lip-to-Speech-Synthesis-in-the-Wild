...
class MultiDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        audio, info["audio_fps"] = librosa.load(
            file_path.replace("video", "audio")[:-4] + ".flac", sr=16000
        )
        ...
        if self.sample_window:
            vid, melspec, spec, audio = self.extract_window(
                vid, melspec, spec, audio, info
            )
        ...
        melspec = nn.ConstantPad2d(
            (0, self.max_v_timesteps * 4 - num_a_frames, 0, 0), 0.0
        )(melspec)
        spec = nn.ConstantPad2d(
            (0, self.max_v_timesteps * 4 - num_a_frames, 0, 0), 0.0
        )(spec)
            ...
            [
                ...
                torch.zeros(
                    [
                        1,
                        int(
                            self.window_size / info["video_fps"] * info["audio_fps"]
                            - aud.size(1)
                        ),
                    ]
                ),
