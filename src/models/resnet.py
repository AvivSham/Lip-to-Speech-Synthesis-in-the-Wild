...
class ResNet(nn.Module):
    def __init__(
        self, block, layers, num_classes=1000, zero_init_residual=True, large_input=True
    ):
        super().__init__()
        self.inplanes = 64
...
def resnet18(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        pretrained_model.fc = nn.Linear(
            pretrained_model.fc.in_features, kwargs["num_classes"]
        )
...
class ResNetModel(nn.Module):
    ...
    def forward(self, x):
        transposed = x.transpose(1, 2).contiguous()  # B,T,C,H,W
        view = transposed.view(-1, 64, x.size(3), x.size(4))  # BT,C,H,W : BT,64,28,28
