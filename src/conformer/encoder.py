...
class ConformerBlock(nn.Module):
    ...
    def forward(self, inputs: Tensor, mask: Tensor) -> Tensor:
        inputs = self.feed_forward1(inputs)
        inputs = self.MHA(inputs, mask)
        inputs = self.Conv(inputs)
        inputs = self.feed_forward2(inputs)
...
class ConformerEncoder(nn.Module):
    ...
    def forward(self, inputs: Tensor, mask: Tensor) -> Tensor:
        ...
        Args:
            ...
            mask: `BoolTensor` of size ``(batch, seq_length)`` True location will be masked with `-inf`
