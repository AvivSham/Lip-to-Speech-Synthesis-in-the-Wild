...
def train_net(args):
    ...
    if args.asr_checkpoint is not None:
        asr_model = ASR_model(
            num_layers=6, num_attention_heads=4, num_class=train_data.num_characters
        )
    else:
        asr_model = None
    ...
    if args.checkpoint is not None:
        ...
        checkpoint = torch.load(
            args.checkpoint, map_location=lambda storage, loc: storage.cuda()
        )
    ...
    if args.asr_checkpoint is not None:
        ...
        checkpoint = torch.load(
            args.asr_checkpoint, map_location=lambda storage, loc: storage.cuda()
        )
    ...
    optimizer = optim.AdamW(
        params, lr=args.lr, weight_decay=args.weight_decay, amsgrad=True
    )
    ...
    for epoch in range(args.start_epoch, epochs):
        ...
        for i, batch in enumerate(dataloader):
            ...
            if i % 100 == 0:
                ...
                prev_time = time.time()
                print(
                    "******** Training [%d / %d] : %d / %d, Iter Time : %.3f sec, Learning Rate of %f ********"
                    % (
                        epoch,
                        epochs,
                        (i + 1) * batch_size,
            ...
            for bb in range(vid.size(0)):
                if random.random() < args.mask_prob:
                    mask_len = min(
                        random.randint(2, 7), vid_len[bb] - start_frame[bb] - 1
                    )  # span length
                    mask_st_frame = random.randint(
                        start_frame[bb], start_frame[bb] + window_size[bb] - mask_len
                    )
                    vid[bb, :, mask_st_frame : mask_st_frame + mask_len] = 0.0
                mel_masks.append([1] * mel_len[bb] + [0] * (mel.size(3) - mel_len[bb]))

            mel_masks = (
                torch.FloatTensor(mel_masks).view(vid.size(0), 1, 1, -1).cuda()
            )  # B, 1, 1, S
            ...
            gen_v_feat = torch.zeros(
                v_feat.size(0), max_window_size, v_feat.size(2)
            ).cuda()
            gen_v_len = []
            ##### selecting window for generation ####
            for bb in range(vid.size(0)):
                temp_v_feat = v_feat[
                    bb, start_frame[bb] : start_frame[bb] + window_size[bb], :
                ]
            ...
            recon_loss = criterion(
                train_data.denormalize(gen_mel), train_data.denormalize(mel.cuda())
            )

            ctc_loss = CTC_criterion(
                ctc_pred.transpose(0, 1).log_softmax(2),
                target.cuda(),
                vid_len,
                target_len,
            ) / vid.size(0)
            ...
            beam_results, beam_scores, timesteps, out_lens = decoder.decode(
                F.softmax(ctc_pred, 2)
            )
            beam_text = [
                train_data.arr2txt(beam_results[_][0][: out_lens[_][0]])
                for _ in range(vid.size(0))
            ]
            truth_txt = [train_data.arr2txt(target[_]) for _ in range(vid.size(0))]
            beam_wer.extend(wer(beam_text, truth_txt))

            if i % 100 == 0:
                wav_pred = train_data.inverse_mel(
                    gen_mel.detach()[0], mel_len[0:1], stft
                )  # 1, 80, T
                wav_gt = train_data.inverse_mel(
                    mel.cuda().detach()[0], mel_len[0:1], stft
                )
            else:
                wav_pred = 0
                wav_gt = 0
            ...
            if writer is not None:
                ...
                if i % 100 == 0:
                    ...
                    writer.add_image(
                        "train_mel/gen",
                        train_data.plot_spectrogram_to_numpy(
                            gen_mel.cpu().detach().numpy()[0]
                        ),
                        step,
                    )
                    writer.add_image(
                        "train_mel/gt",
                        train_data.plot_spectrogram_to_numpy(mel.detach().numpy()[0]),
                        step,
                    )
                    writer.add_audio(
                        "train_aud/pred_mel",
            ...
            if step % args.eval_step == 0:
                ...
                torch.save(
                    {
                        "v_front_state_dict": v_state_dict,
                        "ctc_layer_state_dict": ctc_layer_state_dict,
...
def validate(v_front, mel_layer, sp_layer, fast_validate=False, epoch=0, writer=None):
    with torch.no_grad():
        ...
        description = (
            "Validation on subset of the Val dataset" if fast_validate else "Validation"
        )
        print(description)
        for i, batch in enumerate(dataloader):
            if i % 10 == 0:
            ...
            for _ in range(g_mel.size(0)):
                ...
                try:
                    pesq_list.append(
                        pesq(
                            8000,
                            librosa.resample(wav_tr[_][:min_len].numpy(), 16000, 8000),
                            librosa.resample(wav_pred[_][:min_len], 16000, 8000),
                            "nb",
                        )
                    )
                except:
            ...
            if i in [
                int(required_iter // 3),
                int(2 * (required_iter // 3)),
                int(3 * (required_iter // 3)),
            ]:
                if writer is not None:
                    writer.add_image(
                        "val_mel_%d/gen" % i,
                        val_data.plot_spectrogram_to_numpy(
                            g_mel.cpu().detach().numpy()[0][:, :, : mel_len[0]]
                        ),
                        epoch,
                    )
                    writer.add_image(
                    ...
                    writer.add_audio(
                        "val_aud_%d/mel" % i,
                        wav_gt[0][: len(wav_pred[0])],
                        global_step=epoch,
                        sample_rate=16000,
                    )
                    writer.add_audio(
                        "val_aud_%d/gt" % i,
                        wav_tr[0][: len(wav_pred[0])],
                        global_step=epoch,
                        sample_rate=16000,
                    )
                    fig = plt.figure()
        ...
        return (
            ...
            np.mean(np.array(stoi_list)),
            np.mean(np.array(estoi_list)),
            np.mean(np.array(pesq_list)),
        )
